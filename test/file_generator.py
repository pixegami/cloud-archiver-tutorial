...
def generate_test_set(path: str):
    ...
    generate_test_files(
        1, test_dir_1, days_old=180
    )  # Even though these are old, the folder was touched recently.
...
def generate_test_file(root_path: str, days_old: int = 0):
...
def generate_test_files(n: int, root_path: str, days_old: int = 0):
    for _ in range(n):
        generate_test_file(root_path, days_old)
